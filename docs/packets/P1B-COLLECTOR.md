...
状态：可执行，P1A已验收。只实现采集与本地价表估算，不做 UI 或联网价格更新。
...
依赖使用已准备的 Collector/Cargo.toml 与锁文件；原则上不新增依赖，缺失能力先说明。src/ 与 tests/ 尚未创建，直接创建文件，不读取不存在文件。脚本离线执行测试、Clippy与release构建，不负责真实日志检查或网络价表下载。
