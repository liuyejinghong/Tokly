...
协议v1已定义在 [Shared/protocol.md](../Shared/protocol.md)，以该文件为精确字段契约。根包含版本、生成时间、时区、覆盖范围、hourlyDate、价格时间、daily/hourly数据、来源发现状态及警告。

每日/小时桶都使用“客户端 → 模型 → 五类Token与估算费用”结构。模型费用逐事件在Rust中计算后汇总；Swift共用一套聚合函数计算所选范围、父子合计和显示。日与小时是同一用量的两个投影，禁止相加。根不再发送多份冗余汇总，避免不同视图走出不同口径。

Token全程Int64；价格缺失不等于零价格。无法完整定价的事件保守地整体计入未定价用量，完全未知时amountUsd=null。协议明确这一口径，UI显示部分估算或不可用。
...
菜单栏默认显示今日 Token，点击后看到估算费用、客户端分项、更新时间、刷新和打开详情。详情窗口提供今日/近七天/本月与模型分布。客户端用量可展开到子模型，模型页按客户端分组；同一模型在不同客户端下分别保留。月图展示方案见 [原型评审](PRODUCT-PROTOTYPE.md)，用户已确认默认日曲线、可切换自然周汇总。不在首版添加账户管理。
