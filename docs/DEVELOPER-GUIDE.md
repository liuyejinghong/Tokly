...
> 本文保留实现与验证细节，面向贡献者。产品介绍和当前社区发行状态以 [README](../README.md) 为准。

Tokly 是一个原生 macOS AI 用量统计应用：读取本机客户端记录，展示 Token 用量及按模型单价计算的估算费用，提供主窗口、菜单栏弹窗和桌面小组件。

本仓库为社区源码项目；本文保留实现细节和开发交接说明。它包含实现、固定的上游核心、合成测试和验收记录；不包含用户会话日志、运行时缓存、签名私钥或已编译安装包。不要假定原开发机器上的工具、凭据、临时文件和已安装应用在新的 checkout 中仍然存在。
...
核心复用 [missuo/tokens](https://github.com/missuo/tokens) 的固定提交 `75aba190695c9de5f2695d9baba6afd3c8cb63f8`。来源及原始指纹见 [UPSTREAM.json](../Collector/vendor/UPSTREAM.json)，获准本地补丁见 [LOCAL-PATCHES.json](../Collector/vendor/LOCAL-PATCHES.json)。当前补丁包括本地采集 API 提取、合成测试和 SQLite 构建特性配置；不能静默更新 vendor 或只改哈希来掩盖未审查修改。
...
默认 `build-local.sh` 构建普通社区版本；0.2.0源码同时保留独立Store Check实验目标，见 [沙盒验证记录](APP-STORE.md)。目录授权和OpenCode只读数据库保护仅在实验沙盒配置启用。社区版尚未提供跨机器验证的二进制发行，签名与Widget配置需要在目标环境核验。
