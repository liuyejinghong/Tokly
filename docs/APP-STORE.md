...
> 当前方向：开源社区分发，App Store 上架暂缓。以下保留实验沙盒的实现与验证记录，不是默认社区构建的安装要求。

当前源码为0.2.0/build4开发线，未创建0.2.0发布标签，未提交Apple审核。已安装0.1.0保持不变。
...
- 确认付费Apple Developer Program资格，注册正式App Store记录、分发签名/配置与Archive验证。用户已确认尚无付费会员；当前不推进购买或提交。
