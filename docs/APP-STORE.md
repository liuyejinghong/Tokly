...
本分支为0.2.0/build3开发线，未创建0.2.0发布标签，未提交Apple审核。main及已安装0.1.0保持独立。
...
上述运行验证并非同一时刻的最终组合验收。最新组合还加入了系统SQLite、隐私资源、启动时失效恢复、arm64限制和授权控件细化；最终签名构建的再次运行/重启回归待完成。当前Mac锁屏阻止了这一步，不能将已通过的早期验证误报为最终包全部通过。
...
- 最终组合的授权重启、撤销与失效恢复回归；系统SQLite的只读WAL场景复核。
